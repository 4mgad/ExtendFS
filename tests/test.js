...
var testCase2 = function () {
	...
	xfs.copyDir(currPath + 'js', currPath + '_js', function (err, sd, dd) {
		if (err) {
			console.log(err);
		} else {
...
var testCase4 = function () {
	...
	xfs.deleteDir('nodir', function (err, dirPath) {
		if (err) {
			console.log('SUCCESS!');
			testCase5();
		} else {
...
var testCase5 = function () {
	...
	xfs.copyDir('nosrcdir', 'dest', function (err, sd, dd) {
		if (err) {
			console.log('SUCCESS!');
			testCase6();
		} else {
...
var testCase6 = function () {
	...
	xfs.copyDir(currPath + 'js', currPath + 'js/test', function (err, sd, dd) {
		if (err) {
			console.log('SUCCESS!');
			testCase7();
		} else {
...
var testCase7 = function () {
	...
	var test = function (index) {
		...
		xfs.deleteDir(dirPath.substr(0, dirPath.indexOf('test2')), function (err, deletedDir) {
			xfs.createDirs(dirPath, function (err, createdDir) {
				if (err) {
					console.log(err);
				} else {
					if (createdDir === dirPath && num === 4) {
						xfs.deleteDir(dirPath.substr(0, dirPath.indexOf('test2')), function (err) {
							if (err) {
								console.log(err);
								success = false;
							}
